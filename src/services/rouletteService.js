...
export function buildSpin(seed) {
  return {
    ...
    // Оборотов меньше, но прогон длиннее: дёрганье на разгоне — не нехватка
    // кадров, а строб. При 32 оборотах за 12 секунд колесо в первые мгновения
    // проходит около шестой части круга за кадр, и глаз видит скачки, сколько
    // бы кадров мы ни рисовали. Держим пик в пределах, которые экран способен
    // показать, а ощущение долгой раскрутки добираем временем.
    turns: 13 + Math.floor(deriveFromSeed(seed, "turns") * 8),
    spinSeconds: roundAmount(12 + deriveFromSeed(seed, "duration") * 4),
  };
}
